...
export default function AdminGalleryPage() {
  ...
  async function handleDelete(id: string) {
    if (!confirm('Delete this photo from the website and Cloudinary?')) return;
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '1rem' }}>
        {photos.map(photo => (
          <div key={photo._id} style={{ background: 'rgba(255,252,248,0.95)', border: '1px solid rgba(184,168,138,0.35)', borderRadius: 16, overflow: 'hidden', boxShadow: '0 2px 10px rgba(61,53,41,0.06)' }}>
            <div style={{ position: 'relative', aspectRatio: '4/3' }}>
              <Image src={photo.cloudinaryUrl} alt={photo.caption} fill style={{ objectFit: 'cover' }} sizes="220px" />
            </div>
            <div style={{ padding: '0.75rem' }}>
              ...
              <button onClick={() => handleDelete(photo._id)} style={{ width: '100%', padding: '0.4rem', borderRadius: 8, background: 'rgba(122,24,24,0.08)', border: '1px solid rgba(122,24,24,0.2)', color: '#7A1818', cursor: 'pointer', fontSize: '0.8rem' }}>Delete</button>
            </div>
          </div>
        ))}
      </div>
